...
const VALID_COLLABORATION_MODES = ["off", "warn", "enforce"] as const;
const KNOWN_COLLABORATION_POLICY_KEYS = new Set(["mode", "sla"]);
const KNOWN_SLA_KEYS = new Set([
  "replyByMinutes",
  "remindAtMinutes",
  "escalateAtMinutes",
  "staleTaskHours",
...
export function validateCollaborationPolicy(raw: unknown): string[] {
  const errors: string[] = [];
  const p = "office.collaborationPolicy";
  ...
  if ("mode" in r) {
    if (
      ...
      !VALID_COLLABORATION_MODES.includes(
        r["mode"] as (typeof VALID_COLLABORATION_MODES)[number],
      )
    ) {
      errors.push(
        `${p}.mode must be "off", "warn", or "enforce" (got "${r["mode"]}")`,
      );
    }
  }
  ...
  if ("sla" in r) {
    ...
    if (typeof sla !== "object" || sla === null) {
      errors.push(`${p}.sla must be an object`);
    } else {
      const slaObj = sla as Record<string, unknown>;
      for (const key of Object.keys(slaObj)) {
        if (!KNOWN_SLA_KEYS.has(key)) {
          errors.push(
            `${p}.sla: unknown key "${key}" — known: ${[...KNOWN_SLA_KEYS].join(", ")}`,
          );
        }
      }
      for (const key of KNOWN_SLA_KEYS) {
        if (key in slaObj) {
          const v = slaObj[key];
          if (typeof v !== "number" || v <= 0) {
            errors.push(`${p}.sla.${key} must be a positive number`);
          }
        }
      }
    }
