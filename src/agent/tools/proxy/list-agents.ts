...
export function createListAgentsProxy(selfName: string, hostFetch: HostFetch): AgentTool<any> {
  return {
    ...LIST_AGENTS,
    execute: async () => {
      const res = await hostFetch("/api/agents", null, "GET");
      if (!res.ok) return textResult("Error listing agents.");
      const agents = await res.json() as Array<{ name: string; status: string; description: string }>;
