# agent-office — full reference

> This is the complete reference. For setup, start with the [README](../README.md).
...
- Any CLI command runs as `docker compose run --rm -it agent-office <command>`, e.g. `oauth login anthropic --office my-office`. OAuth flows that open a local callback server (`openai-codex`) cannot complete inside the container — log in on the host and copy `offices/<id>/oauth/` over.
- `UI_HOST` sets the UI bind address outside Docker too (default `127.0.0.1`).
