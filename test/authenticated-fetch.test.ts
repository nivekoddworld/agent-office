...
import { validateFetchParams, buildAuthHeader } from "../src/agent/tools/fetch-helpers.js";
...
describe("buildAuthHeader", () => {
  ...
  it("allows custom header name from allowlist", () => {
    const h = buildAuthHeader("key123", { mode: "raw", headerName: "X-API-Key" });
    expect(h).toEqual({ name: "X-API-Key", value: "key123" });
  });

  it("rejects disallowed header name", () => {
    expect(() => buildAuthHeader("val", { headerName: "X-Custom" })).toThrow("not allowed");
  });

  it("rejects invalid auth mode", () => {
    expect(() => buildAuthHeader("val", { mode: "digest" })).toThrow("Invalid auth mode");
  });
});
...
describe("validateFetchParams — URL validation", () => {
  it("rejects invalid URLs", async () => {
    const r = await validateFetchParams({ url: "not-a-url", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toBe("Invalid URL");
  });

  it("rejects non-HTTPS URLs", async () => {
    const r = await validateFetchParams({ url: "http://example.com", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toContain("HTTPS");
  });

  it("allows HTTPS URLs", async () => {
    const r = await validateFetchParams({ url: "https://api.github.com/repos", secretName: "S" }, "val");
  ...
  it("rejects localhost HTTP without flag", async () => {
    const r = await validateFetchParams({ url: "http://localhost:3000", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
  });
});
...
describe("validateFetchParams — SSRF protection", () => {
  it("blocks private IP 10.x", async () => {
    const r = await validateFetchParams({ url: "https://10.0.0.1/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toContain("private");
  });

  it("blocks private IP 172.16.x", async () => {
    const r = await validateFetchParams({ url: "https://172.16.0.1/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
  });

  it("blocks private IP 192.168.x", async () => {
    const r = await validateFetchParams({ url: "https://192.168.1.1/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
  });

  it("blocks loopback 127.x", async () => {
    const r = await validateFetchParams({ url: "https://127.0.0.1/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
  });

  it("blocks link-local / metadata 169.254.x", async () => {
    const r = await validateFetchParams({ url: "https://169.254.169.254/latest/meta-data", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
  });

  it("blocks 0.0.0.0", async () => {
    const r = await validateFetchParams({ url: "https://0.0.0.0/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
  });

  it("blocks domain resolving to private IP (DNS layer)", async () => {
    vi.mocked(dns.resolve4).mockResolvedValueOnce(["10.0.0.5"]);
    const r = await validateFetchParams({ url: "https://evil.example.com/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
  ...
  it("blocks domain resolving to loopback (DNS layer)", async () => {
    ...
    const r = await validateFetchParams({ url: "https://rebind.example.com/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toContain("resolves to private");
  });
  ...
  it("blocks IPv4-mapped IPv6 loopback (::ffff:127.0.0.1)", async () => {
    ...
    const r = await validateFetchParams({ url: "https://mapped.example.com/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toContain("resolves to private");
  });
  ...
  it("blocks IPv4-mapped IPv6 private (::ffff:10.0.0.1)", async () => {
    ...
    const r = await validateFetchParams({ url: "https://mapped2.example.com/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toContain("resolves to private");
  });
  ...
  it("blocks hex-form IPv4-mapped IPv6 loopback (::ffff:7f00:1)", async () => {
    ...
    const r = await validateFetchParams({ url: "https://hex.example.com/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toContain("resolves to private");
  });
  ...
  it("blocks hex-form IPv4-mapped IPv6 private (::ffff:a9fe:a9fe = 169.254.169.254)", async () => {
    ...
    const r = await validateFetchParams({ url: "https://metadata.example.com/api", secretName: "S" }, "val");
    expect(r.ok).toBe(false);
    if (!r.ok) expect(r.error).toContain("resolves to private");
  });
...
describe("validateFetchParams — method and headers", () => {
  it("defaults method to GET", async () => {
    const r = await validateFetchParams({ url: "https://api.example.com", secretName: "S" }, "val");
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.result.method).toBe("GET");
  });

  it("rejects invalid method", async () => {
    const r = await validateFetchParams({ url: "https://api.example.com", secretName: "S", method: "TRACE" }, "val");
    expect(r.ok).toBe(false);
  ...
  it("strips blocked headers", async () => {
    const r = await validateFetchParams({
      ...
      secretName: "S",
      headers: { Host: "evil.com", "Content-Type": "application/json", Cookie: "session=abc" },
    }, "val");
    expect(r.ok).toBe(true);
    if (r.ok) {
  ...
  it("auth header cannot be overridden by user headers", async () => {
    ...
    expect(r.ok).toBe(true);
    if (r.ok) expect(r.result.headers.Authorization).toBe("Bearer injected-secret");
  });
});
...
describe("createAuthenticatedFetchTool", () => {
  ...
  it("returns error for unknown secret name", async () => {
    ...
    const result = await tool.execute("id", { url: "https://api.example.com", secretName: "MISSING" });
  ...
  it("rejects MODEL_API_KEY as secret name", async () => {
    ...
    const result = await tool.execute("id", { url: "https://api.example.com", secretName: "MODEL_API_KEY" });
    expect(getText(result)).toContain("cannot be used with authenticated_fetch");
  });

  it("makes authenticated GET request", async () => {
    const mockFetch = vi.fn(async () => new Response("ok", { status: 200, statusText: "OK" }));
    globalThis.fetch = mockFetch as any;

    const tool = createAuthenticatedFetchTool(secrets);
    const result = await tool.execute("id", { url: "https://api.github.com/repos", secretName: "GITHUB_TOKEN" });
  ...
  it("makes POST request with body", async () => {
    const mockFetch = vi.fn(async () => new Response('{"id":1}', { status: 201, statusText: "Created" }));
    globalThis.fetch = mockFetch as any;
  ...
  it("redacts secret value from response body", async () => {
    ...
    globalThis.fetch = vi.fn(async () => new Response(leakyBody, { status: 200, statusText: "OK" })) as any;
