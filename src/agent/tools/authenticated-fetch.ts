...
import { validateFetchParams, FETCH_TIMEOUT_MS, MAX_RESPONSE_BODY, RESERVED_SECRET_NAMES, type FetchParams } from "./fetch-helpers.js";
  ...
  return {
    ...
    execute: async (_id, params: FetchParams) => {
      if (RESERVED_SECRET_NAMES.has(params.secretName)) {
        return textResult(`Error: Secret "${params.secretName}" cannot be used with authenticated_fetch.`);
      }
      const secretValue = resolvedSecrets[params.secretName];
      if (!secretValue) {
        return textResult(`Error: Secret "${params.secretName}" not configured for this agent.`);
      }
      ...
      try {
        ...
        if (Buffer.byteLength(responseBody, "utf-8") > MAX_RESPONSE_BODY) {
          return textResult("Error: Response body too large (exceeds 5 MB limit).");
        }

        const safeBody = redactText(responseBody, [secretValue]);
        return textResult(`HTTP ${res.status} ${res.statusText}\n\n${safeBody}`);
      } catch (err) {
